...
const AllMuzis = ({ muziService, commentService, username, addable }) => {
  ...
  const onAddMuzi = (text) => {
    console.log('onaddmuzi');
    muziService //
      .postMuzi(text)
      .then((muzi) => {
        return setMuzis([...muzis, muzi]);
      });
  };

  const onAddMuziComment = (MuziId, text) => {
    commentService //
      .postComment(MuziId, text)
      .then((data) => {
        setComments([...comments, data]);
      });

    muziCommentCountUpdate();
  };

  //댓글 업데이트 되면 게시판 댓글수 갱신해주기
  const muziCommentCountUpdate = async () => {
    muziService //
      .getMuzis(username)
      .then((muzis) => {
        return setMuzis([...muzis]);
